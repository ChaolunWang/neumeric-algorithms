...
template<class U>
friend Matrix<U> solve(const Matrix<U> &A, const Matrix<U> &b);                 //matrix solving LU
...
public:
	...
	Matrix<T> operator*(const Matrix<T> &m) const;                                //matrix multiplication
	Matrix<T> operator*(double value) const;                                      //matrix multiplication
	void init(T a);                                				//matrix initialize
	Matrix<T> inv() const;                                			//matrix inverse only for symmetric
	Matrix<T> t() const;                                			//matrix transpose
	Matrix<T> operator+(const Matrix<T> &m) const;                                //matrix +
	Matrix<T> operator-(const Matrix<T> &m) const;                                //matrix -
	double len();                                			        //matrix eucledian length
	void truncate(int r, int c);                                		//matrix truncation
	...
	Row operator[](int m) const{return Row(&mMatrix[m*mColumn]);}              //first[] operator for read
...
//save matrix in ostream
{
	...
	for(int i=0; i<matrix.mRow; ++i)
	{
		...
		for(int j=0; j<matrix.mColumn; ++j)
			os<<setw(7)<<left<<matrix.mMatrix[i*matrix.mColumn+j]<<' ';
		os<<'\n';
	}
	//os<<'\n';
	os.flags(fstate);
...
Matrix<T> solve(const Matrix<T> &A, const Matrix<T> &b)
{
	...
	Matrix<T> result(n, 1);
	for(int i=0; i<n; ++i)
		result.mMatrix[i]=b.mMatrix[i];
	Matrix<T> AT=A.t();
	dgesv_(&n, &ks, &AT.mMatrix[0], &n, &ipiv[0], &result.mMatrix[0], &n, &info);

  	if (info != 0) cout << "Error in solve linear system: Flag is " << info << endl;
	return result;	
}
...
template<class T>
void Matrix<T>::truncate(int r, int c)
{
	mRow-=r;
	mColumn-=c;
}
