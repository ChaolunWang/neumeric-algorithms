...
# include <ctime>
# include <cmath>
# include <omp.h> //first modification, add reference to include openmp library
using namespace std;
...
//
  ctime1 = omp_get_wtime(); //second change record the initial time 
  ...
  ctime2 = omp_get_wtime(); //second change record the finish time
...
//
{
  ...
  pe = 0.0;
  ke = 0.0;
  #pragma omp parallel private(k, i, j, d, d2) shared (np, nd)
  #pragma omp for reduction(+:pe,ke) //third change to parralell the for loop
  for ( k = 0; k < np; k++ )
  {
//
