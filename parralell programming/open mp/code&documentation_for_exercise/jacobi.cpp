...
//
{
  ...
  int i;
  int j;
  int n = 500;  //setted as big number for timing
  double row_sum;
  int seed;
  double *x;
  double *x_old;
...
#pragma omp parallel private(row_sum,i,j) shared (n, b , a, x)
{
#pragma omp for //reduction(+:row_sum)
  for ( i = 0; i < n; i++ )
  {
    row_sum = 0.0;
    for ( j = 0; j < n; j++ )
...
//
{
  ...
  for ( it = 1; it <= it_max; it++ )
  {
    ...
    #pragma omp parallel private(i,axi,j) shared (n)
    {
      #pragma omp for //copy x to x_old loop added
      for ( i = 0; i < n; i++ )
      {
        x_old[i] = x[i];
      }

      #pragma omp for //compute new value of x loop added
      for ( i = 0; i < n; i++ )
      {
        axi = 0.0;
      ...
      diff = 0.0;
      #pragma omp for reduction(+:diff) //compute ||x-x_old|| loop added
      for ( i = 0; i < n; i++ )
      {
  ...
  cout << "\n";
  cout << "  First 10 elements of estimated solution:\n";
  cout << "\n";
